# Exercise 5: Checkout with Validation
...
In this exercise, you will fix this by adding a "checkout" screen that requires customers to enter a valid address.

## Task 1: Inserting checkout into the flow

Start by adding a new razor file called `Checkout.razor` to the `Pages` folder of the `BlazingPizza.client` project. 

We will include the below directives at the top of the file including a `@page` directive matching the URL `/checkout` so that it can compile, just like you did in `Index.razor`. 
...
For the initial markup, let's display the details of the order using your `OrderReview` component:
...
To implement `PlaceOrder`, copy the method with that name from `Index.razor` into the `@code` block of `Checkout.razor`:
...
Next, let's bring customers here when they try to submit orders. Back in `Index.razor`, make sure you've deleted the `PlaceOrder` method, and then change the order submission button, `<button class="btn btn-warning" disabled="@(OrderState.Order.Pizzas.Count == 0)" @onclick="PlaceOrder">` into a regular HTML link to the `/checkout` URL, i.e.:
...
Now, when you run the app, you should be able to reach the checkout page by clicking the *Order* button after placing an order, and from there you can click *Place order* to confirm it.

![Confirm order](images/77242251-d2530780-6bb9-11ea-8535-1c41decf3fcc.png)

## Task 2: Capturing the delivery address
...
Create a new razor file in the `BlazingPizza.Client` project's `Shared` folder called `AddressEditor.razor`. It's going to be a general way to edit `Address` instances, so have it receive a parameter of this type in the `@code` block
...
## Task 3: Adding server-side validation
...
Open `Address.cs` in the `BlazingPizza.Shared` project, and put a `[Required]` attribute onto each of the properties except for `Id` (which is autogenerated, because it's the primary key) and `Line2`, since not all addresses need a second line. Do this by adding first adding the `using` statement shown below to the top of the file. Then replace the `Address` class with what's shown below.  You can also place some `[MaxLength]` attributes if you wish, or any other `DataAnnotations` rules:
...
Now, after you recompile and run the app, you should be able to observe the validation rules being enforced on the server. If you try to submit an order with a blank delivery address, then the server will reject the request and you'll see an HTTP 400 ("Bad Request") error in the *Network* tab of your browser's developer tools:
...
## Task 4: Adding client-side validation
...
One of the most important built-in UI components for data entry is the `EditForm`. This renders as an HTML `<form>` tag, but also sets up an `EditContext` to track what's going on inside the form. To use this, go to your `Checkout.razor` file, and wrap an `EditForm` around the whole of the contents of the `main` div:
...
If you ran the app now, you could still submit a blank form (and the server would still respond with an HTTP 400 error). That's because your `<button>` isn't actually a `submit` button. Modify the `button` by adding `type="submit"` and `removing` its `@onclick` attribute entirely as shown below:

```
<button type="submit" class="checkout-button btn btn-warning" disabled="@isSubmitting">
```

![Button submit](images/ButtonSubmit.png)
...
![Edit Form Attribute](images/EditFormAtt.png)

To fix the errors, we need to add a variable for `isSumbitting` to the file's `@code` block:

```
bool isSubmitting;
```

We also need to add the following statement to the `Placeholder` method in the `@code` block.

```
isSubmitting = true;
```

![Code Block submitting](images/CodeBlockSubmitting.png)
...
![Validation summary](images/77242430-9d47b480-6bbb-11ea-96ef-8865468375fb.png)
...
Start by removing the `<ValidationSummary>` component entirely. Then, switch over to `AddressEditor.razor`, and add separate `<ValidationMessage>` components next to each of the form fields. 
...
also change `@bind` to `@bind-Value`. All these changes are shown below. 
...
![Input components](images/77242542-ba30b780-6bbc-11ea-8018-be022d6cac0b.png)
